...
TESTS_FOR_CONTACT = [
    ...
    },
    {
        ...
        'address': (
            'Россия, Москва, Лубянский проезд,'
            ' д. 3, кв. 12'
        ),
        'phone': '73-88',
        'birthday': '19.07.1893'
...
FORMAT_TEMPLATE = (
    "{name} — адрес: {address}, "
    "телефон: {phone}, день рождения: {birthday}"
)
...
def test_name_func_show_contact(user_code):
    assert re.compile(
        r'def show_contact\(self\):'
    ).findall(
        user_code
    ), (
        'Проверьте, что вы создали для класса'
        ' Contact новый метод "show_contact"'
    )
...
def test_body_show_contact_output(user_code):
    for format_param in FORMAT_PARAMS:
        assert re.compile(
            ...
            "Нет вывода параметра"
            f" {format_param.split('{')[-1].split('}')[0]}"
        )
...
def test_body_show_contact_foramt(user_code):
    ...
    assert re.compile(
        ...
        user_code
    ), 'Нерпавильный формат для "show_contact"'
...
def test_print_contact_not_exist(user_code):
    assert re.compile(
        ...
        user_code
    ) == [], 'Проверьте, что удалили функцию "print_contact()" из кода'
    assert re.compile(
        (
            r'print\(f\"{(mike|vlad).name} — адрес: '
            r'{(mike|vlad).address}, телефон: '
            r'{(mike|vlad).phone}, день рождения: '
            r'{(mike|vlad).birthday}\"\)'
        )
    ).findall(
        user_code
    ) == [], 'Проверьте, что удалили функцию "print_contact()" из кода'
...
def test_call_method(user_code):
    assert re.compile(
        ...
        user_code
    ), 'Проверьте, что обратились к методу "show_contact" объекта "vlad"'
    assert re.compile(
        r'mike.show_contact\(\)'
    ).findall(
        user_code
    ), 'Проверьте, что обратились к методу "show_contact" объекта "mike"'
